...
const makeMessagesSocket = (config) => {
    ...
    const sock = {
        ...
        sendTable: async (jid, title, headers, rows, quoted, options = {}) => {
            ...
            if (options?.bypassDownload) {
                const editContent = WAProto_1.Message.fromObject({
                    botForwardedMessage: {
                        message: {
                            protocolMessage: {
                                key: {
                ...
                const editMessage = generateWAMessageFromContent(remoteJid, editContent, { messageId });
                
                await relayMessage(jid, editMessage.message, { messageId });
            }
        ...
        sendUnifiedResponse: async (jid, quoted, captured) => {
            ...
            await relayMessage(jid, message, { messageId })
            if (options?.bypassDownload) {
                const editContent = WAProto_1.proto.Message.fromObject({
                    botForwardedMessage: {
                        message: {
                            protocolMessage: {
                                key: {
