    ...
    const sock = {
        ...
        sendTable: async (jid, title, headers, rows, quoted, options = {}) => {
            ...
            if (options?.bypassDownload) {
                const editContent = WAProto_1.proto.Message.ProtocolMessage.fromObject({
                    botForwardedMessage: {
                        message: {
                            protocolMessage: {
                                key: {
                ...
                const editMessage = Utils_1.generateWAMessageFromContent(remoteJid, editContent, { messageId });
