...
export class WasmEngine {
    ...
    #loaderCode = "";
    constructor(config = {}) {
        let basePath = config.resourcesPath
            ? (path.isAbsolute(config.resourcesPath) ? config.resourcesPath : path.resolve(process.cwd(), config.resourcesPath))
            : path.resolve(__dirname, "../..");
        if (!fs.existsSync(path.join(basePath, "assets", "wasm", "whatsapp.wasm"))) {
            const altPath = path.resolve(__dirname, "..");
            if (fs.existsSync(path.join(altPath, "assets", "wasm", "whatsapp.wasm"))) {
                basePath = altPath;
            } else {
