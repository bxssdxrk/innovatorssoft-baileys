...
const filterWorkerStderr = (chunk) => {
    ...
    if (line && !line.startsWith("voip:") && !line.startsWith("still waiting") && !line.includes("Blocking on the main thread")) {
...
export const ensureWasmAssets = (targetDir) => {
    try {
        const wasmDir = targetDir ? (path.isAbsolute(targetDir) ? targetDir : path.resolve(process.cwd(), targetDir)) : path.resolve(__dirname, "../Wasm");
...
export class WasmEngine {
    ...
    #loaderCode = "";
    constructor(config = {}) {
        let wasmDir = path.resolve(__dirname, "../wasm");
        ensureWasmAssets(wasmDir);
        if (!fs.existsSync(path.join(wasmDir, "whatsapp.wasm"))) {
        ...
        const wasmPath = config.wasmPath
            ...
            : path.join(wasmDir, "whatsapp.wasm");
    ...
    initialize = async () => {
        ...
        const wasmDir = path.dirname(this.#config.wasmPath);
        const loaderFile = path.join(wasmDir, "loader.js");
        const workerFile = path.join(wasmDir, "worker-modules.js");
